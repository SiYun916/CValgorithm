...
cv::Mat agc_mean_mix(cv::Mat& src, int kernel_size = -1, float average = 0.5) {
    cv::Mat src_hsv;
...
Mat test(Mat src) {
    Mat blue,white,bule_noHalo;
    inRange(src, Scalar(230, 0, 0), Scalar(255, 255, 255), white);
    cvtColor(src, src,COLOR_BGR2HSV);
    inRange(src, Scalar(80, 40, 100), Scalar(150, 255, 255), blue);
    bitwise_and(blue,white, bule_noHalo);
    imwrite("C:\\Users\\SiYun\\Pictures\\BlenderRender\\1\\1.png", bule_noHalo);
...
int main(int argc, char **agrv) {
    string ori_path, save_path;
    ori_path = "C:\\Users\\SiYun\\Desktop\\gamma\\1.png";
    save_path = "C:\\Users\\SiYun\\Desktop\\gamma\\1HE.png";

    Mat image = imread(ori_path);
//    PerfectReflection pr = PerfectReflection(image, 0.1);
//    Mat res = pr.Process();
    HistogramEqual he = HistogramEqual(image);
    Mat res = he.Process();
